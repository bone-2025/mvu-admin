...
export const Route = createFileRoute("/_console/users/$userId")({
  loader: async ({ context: { queryClient, credentials }, params }) => {
    ...
    const phonePromise = queryClient.ensureQueryData(
      userPhonesQuery(credentials.serverName, params.userId),
    );
...
function PhoneListItem({
  ...
  const intl = useIntl();
  const { mutate, isPending } = useMutation({
    mutationFn: (userPhoneId: Ulid) =>
      deleteUserPhone(queryClient, serverName, userPhoneId),
    onError: () => {
      toast.error(
        intl.formatMessage({
          id: "pages.users.delete_phone.error",
          defaultMessage: "Failed to delete phone number",
          description: "The error message for deleting a phone number",
        }),
      );
    },
    onSuccess: async (): Promise<void> => {
      toast.success(
        intl.formatMessage({
          id: "pages.users.delete_phone.success",
          defaultMessage: "Phone number deleted",
          description: "The success message for deleting a phone number",
        }),
      );
      await queryClient.invalidateQueries({
        queryKey: ["mas", "user-phones", serverName, attributes.user_id],
      });
    },
  });
  ...
  return (
    <Data.Item>
      <Data.Title>
        <FormattedMessage
          id="pages.users.phone_number"
          defaultMessage="手机号"
          description="Phone number label"
        />
      </Data.Title>
      <Data.Value>{attributes.phone}</Data.Value>
      <Dialog.Root
        trigger={
          <Button
            destructive
      ...
      >
        <Dialog.Title>
          <FormattedMessage
            id="pages.users.delete_phone.title"
            defaultMessage="Remove phone number?"
            description="The title of the modal asking for confirmation to delete a phone number"
          />
        </Dialog.Title>
        <Dialog.Description>
          <FormattedMessage
            id="pages.users.delete_phone.description"
            defaultMessage="Are you sure you want to remove this phone number ({phone}) from this user ({mxid})?"
            description="The description of the modal asking for confirmation to delete a phone number"
            values={{
...
function PhonesList({ userId, mxid, serverName }: PhonesListProps) {
  const intl = useIntl();
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);

  const { data: phonesData } = useSuspenseQuery(
    userPhonesQuery(serverName, userId),
  );
  // TODO: handle error message from the server
  const { mutate, isPending } = useMutation({
    mutationFn: (phone: string) =>
      addUserPhone(queryClient, serverName, userId, phone),
    onError: () => {
      toast.error(
        intl.formatMessage({
          id: "pages.users.add_phone.error",
          defaultMessage: "Failed to add phone number",
          description: "The error message for adding a phone number",
        }),
      );
    },
    onSuccess: async (): Promise<void> => {
      toast.success(
        intl.formatMessage({
          id: "pages.users.add_phone.success",
          defaultMessage: "Phone number added",
          description: "The success message for adding a phone number",
        }),
      );
      await queryClient.invalidateQueries({
        queryKey: ["mas", "user-phones", serverName, userId],
      });
      setOpen(false);
    },
  });
      ...
      {phonesData.data.length === 0 && (
        <Dialog.Root
          ...
          trigger={
            ...
            >
              <FormattedMessage
                id="pages.users.add_phone.button"
                defaultMessage="Add phone number"
                description="The label of the button for adding a phone number to a user"
              />
            </Button>
          }
        >
        <Dialog.Title>
          <FormattedMessage
            id="pages.users.add_phone.title"
            defaultMessage="Add phone number"
            description="The title of the modal for adding a phone number"
          />
        </Dialog.Title>
        <Dialog.Description>
          <FormattedMessage
            id="pages.users.add_phone.description"
            defaultMessage="Add a phone number to this user"
            description="The description of the modal for adding a phone number"
          />
        </Dialog.Description>
        <Form.Root onSubmit={onSubmit}>
          <Form.Field name="phone">
            ...
            <Form.ErrorMessage match="valueMissing">
              <FormattedMessage
                id="pages.users.add_phone.error.phone_missing"
                defaultMessage="This field is required"
                description="Error message for missing phone number in the modal for adding a phone number"
              />
            </Form.ErrorMessage>
            <Form.ErrorMessage match="patternMismatch">
              <FormattedMessage
                id="pages.users.add_phone.error.phone_invalid"
                defaultMessage="Invalid phone number (must be 11 digits starting with 1)"
                description="Error message for invalid phone number in the modal for adding a phone number"
              />
            </Form.ErrorMessage>
          </Form.Field>
          <Form.Submit disabled={isPending}>
...
function RouteComponent() {
  ...
  const { data: phonesData } = useSuspenseQuery(
    userPhonesQuery(credentials.serverName, userId),
  );
  const { data: emailsData } = useSuspenseQuery(
    ...
    <Navigation.Details>
      ...
      <div className="flex flex-col gap-8">
        ...
        <Data.Grid>
          ...
          {firstPhone && (
            <Data.Item>
              <Data.Title>手机号</Data.Title>
              <Data.Value>{firstPhone.attributes.phone}</Data.Value>
            </Data.Item>
          )}
          ...
          {hasEmail && (
            <Data.Item>
              ...
              <Data.Value>{emailsData.data[0].attributes.email}</Data.Value>
            </Data.Item>
          )}
