...
export const userPhonesQuery = (serverName: string, userId: string) =>
  queryOptions({
    queryKey: ["mas", "user-phones", serverName, userId],
    queryFn: async ({ client, signal }) => {
      const result = await api.listUserPhones({
        ...(await masBaseOptions(client, serverName, signal)),
        query: {
          "filter[user]": userId,
          "page[first]": 10,
...
export const deleteUserPhone = async (
  ...
  const result = await api.deleteUserPhone({
    ...(await masBaseOptions(queryClient, serverName, signal)),
    path: { id: phoneId },
  });
  ensureNoError(result);
...
export const addUserPhone = async (
  ...
  phone: string,
  signal?: AbortSignal,
): Promise<api.SingleResponseForUserPhone> => {
  const result = await api.addUserPhone({
    ...(await masBaseOptions(queryClient, serverName, signal)),
    body: {
      phone,
      user_id: userId,
