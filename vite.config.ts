...
function vitePluginDevelopmentConfig(): Plugin {
  return {
    ...
    transformIndexHtml(html: string): string {
      // Only inject config in development mode if not already set
      if (html.includes("APP_CONFIG_PLACEHOLDER")) {
        // Get server name from environment variable or use default
        const serverName =
          process.env.VITE_SERVER_NAME || process.env.SERVER_NAME || "";
        const config = {
          serverName: serverName || null,
        };
        const configBase64 = Buffer.from(JSON.stringify(config)).toString(
          "base64",
        );
        return html.replace("APP_CONFIG_PLACEHOLDER", configBase64);
      }
      return html;
    },
  };
